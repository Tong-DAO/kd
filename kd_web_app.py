...
st.set_page_config(
    page_title="REEs Soil Kd Value Visualization", # <-- 标题修正
    page_icon="🌱",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.title("🌱 REEs Soil Kd Value Visualization System") # <-- 标题修正

# ==================== Data Loading Functions (Fully English) ====================

@st.cache_resource(show_spinner="Syncing data files from the cloud...") # <-- 英文翻译
def get_hf_file_path(filename_in_repo):
    ...
    try:
        ...
        st.error(f"Failed to download file '{filename_in_repo}' from Hugging Face. Error: {e}") # <-- 英文翻译
        st.error(f"Please check: 1. Repo ID '{HF_REPO_ID}' is correct. 2. File '{full_path_in_repo}' exists in the repo. 3. HUGGING_FACE_HUB_TOKEN is set correctly in Streamlit Cloud secrets.") # <-- 英文翻译
        st.stop()


@st.cache_data(show_spinner="Loading and parsing raster data...") # <-- 英文翻译
def load_raster_data(filename_in_repo):
    ...
    try:
        ...
        st.error(f"Error loading raster file '{local_file_path}' with Rasterio: {str(e)}") # <-- 英文翻译
        return None
    ...
    try:
        ...
        for param_name, filename in param_files.items():
            try:
                ...
                st.toast(f"Failed to load auxiliary parameter: {param_name}", icon="⚠️") # <-- 英文翻译
        ...
        try:
            ...
            st.toast("Failed to calculate Ionic Strength (IS)", icon="⚠️") # <-- 英文翻译
        return params, (row, col)
    except Exception as e:
        st.error(f"Error getting point parameters: {str(e)}") # <-- 英文翻译
        return None
...
def create_map_image(display_data, vmin, vmax, element, depth, norm_method, data_info, marker_point=None):
    try:
        ...
        st.error(f"Error creating map image: {str(e)}") # <-- 英文翻译
        return None
...
with col_left:
    ...
    if data_info is None:
        st.error("Could not load data file. Please refresh the page to try again.") # <-- 英文翻译
        st.stop()
    ...
    try:
        ...
        with st.spinner('Generating map...'): # <-- 英文翻译
        ...
        else: st.error("Failed to generate map image.") # <-- 英文翻译
        
    except Exception as e:
        st.error(f"An error occurred during map generation: {str(e)}") # <-- 英文翻译
...
with col_right:
    ...
    if 'query_result' in st.session_state and st.session_state['query_result'] is not None:
        ...
        else: st.info("👆 Enter coordinates and click 'Query Point'.") # <-- 英文翻译
        st.markdown("**📊 Soil Parameters**")
...
st.markdown("---")
st.markdown("<div style='text-align: center; color: gray; font-size: 12px;'>🌱 REEs Soil Kd Visualization System v2.2 | English Interface<br>Data hosted on Hugging Face Hub | Fetches data on demand</div>", unsafe_allow_html=True) # <-- 英文翻译